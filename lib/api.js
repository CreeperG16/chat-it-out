...
async function sendMessage(payload, token) {
    const ENDPOINT = "https://wkuimailekpioxrlteqk.supabase.co/rest/v1/messages";

    const { data, error } = await post(ENDPOINT, payload, token);
    if (error) {
        return { data: null, error: { message: "Failed to send message", details: error } };
    }
